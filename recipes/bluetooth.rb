...
new_menu_extras = old_menu_extras.split("\n").select { |line| line !~ /Bluetooth.menu/ }.insert(1,'"/System/Library/CoreServices/Menu Extras/Bluetooth.menu",').join("\n")

execute 'Bluetooth should appear in the status bar' do
...
execute "make sure it exists" do
  command 'defaults read com.apple.systemuiserver menuExtras | grep Bluetooth.menu'
  user node['sprout']['user']
end
